...
def test_enrich_from_api_defaults_off_and_round_trips(tmp_path, monkeypatch):
    # v1.5: osu! API enrichment is opt-in (default OFF) and persists.
    monkeypatch.setattr(config, "_config_file", lambda: tmp_path / "config.json")
    assert config.Config(root="x").enrich_from_api_enabled is False
    cfg = config.load_config()                         # no file yet → default
    assert cfg.enrich_from_api_enabled is False
    cfg.enrich_from_api_enabled = True
    config.save_config(cfg)
    assert config.load_config().enrich_from_api_enabled is True
